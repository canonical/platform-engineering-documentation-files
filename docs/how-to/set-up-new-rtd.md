...
The `.readthedocs.yaml` file is a static file — it is identical across all
...
- **PR optimization**: Skips building pull requests that do not touch the
...
| `old_domain` | Old RTD domain to redirect from (for example, `canonical-discourse-k8s-charm.readthedocs-hosted.com`). Leave empty to skip link rewriting. |
...
1. Ensure your RTD project is configured to build from your repository.
   (This step happens in the RTD web UI — it is outside the scope of this template.)
